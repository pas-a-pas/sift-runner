...
def read_features_from_file(filename):
    ...
    f = loadtxt(filename)
    return f[:, :4], f[:, 4:] # feature location, descriptors
...
def plot_feature(im, locs, circle=False):
    ...
    if circle:
        ...
        plot(locs[:, 0], locs[:, 1], '.c')
    axis('off')

def plot_matches(im1, im2, locs1, locs2, matchscores, show_below=True):
    """ Show a figure with lines joining the accepted matches
    input: im1, im2(images as array), locs1, locs2(feature locations),
    matchscores(as output from 'match()'), show_below(if images should be shown below matches). """
